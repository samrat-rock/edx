...
import CollageCard from '@/app/components/CollageCard/page'
...
export default function CollegeDetail() {
  ...
  const college = colleges.find((college) => college.slug === id);

  if (!college) {
    return <div className="p-6 text-red-600 text-lg font-semibold">College not found.</div>;
  }

  const infoColumns = [
    [
      { icon: <LiaUniversitySolid />, text: college.affiliation },
      { icon: <FaCalendarAlt />, text: `Established ${college.establishmentYear} AD` },
      {  text: college.type },
    ],
    [
      { icon: <FaGraduationCap />, text: `${college.graduationRate} Graduation Rate` },
      { icon: <HiOutlineLocationMarker />, text: college.location },
      { icon: <IoPhonePortraitOutline />, text: college.phone },
    ],
    [
      { icon: <GiRotaryPhone />, text: "01-4441577" },
      { icon: <FaRegEnvelope />, text: college.email },
      { icon: <FaGlobeAmericas />, text: college.website },
    ],
  ];
    ...
    <section className="bg-gray-200 p-3">
      <div className="bg-white rounded">
        <div className="relative">
          <div
            className="bg-cover bg-center bg-no-repeat h-52 rounded flex items-center justify-center"
          ...
          <div className="flex items-center justify-between mt-4 px-4 w-full">
            <div className="flex items-center">
              ...
              <div className="lg:pl-52 py-3">
                <h3 className="lg:text-3xl font-semibold">{college.name}</h3>
                <p className="text-[#606060]">{college.affiliation}</p>
              </div>
            </div>
              ...
              <div key={colIdx}>
                <ul>
                  {column.map((item, idx) => (
                    <li
                      key={idx}
                      className="py-2 flex items-center gap-3 text-[#4E4E4E]"
                    >
                      <span className="text-[#E66E19] text-xl">{item.icon}</span>
                      {item.text}
                    </li>
                  ))}
                </ul>
          ...
          <div className="flex gap-6">
            <button
              className={`px-6 py-2 rounded ${
                activeTab === "about" ? "bg-[#FDF1E9] text-[#E97E33]" : "text-[#606060]"
              }`}
